      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            type="primary"
            size="mini"
            @click="detailById(scope.row.id)"
            >查看</el-button
          >

          <el-button type="primary" size="mini" @click="updateSchedule(scope.row.hoscode)">排班</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 进入详情页
    detailById(id) {
      this.$router.push("/hospSet/hospital/show/" + id);
    },
    // 进入排班页
    updateSchedule(hoscode) {
      this.$router.push("/hospSet/hospital/schedule/" + hoscode);
    }
  },
};
</script>
